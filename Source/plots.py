...
##############################################################
from code.plotter import Histograph, ScatterPlot, PieChart
...
select 
	cast(cast( (count(*) / commits.cnt ) as decimal(3,2)) * 100 as unsigned integer)
from
    (select 
    ...
    group by c1.repository_id) commits
        inner join
    commit c2 ON c2.repository_id = commits.repository_id
group by c2.repository_id, c2.committer
'''
